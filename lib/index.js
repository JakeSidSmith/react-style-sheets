...
(function () {
  ...
  function createStyles (tag, style) {
    ...
    if (tag.indexOf('-') >= 0) {
      throw new Error('Found "-" in a tag or class name. Tag and class names must be camelcase.');
    }
  ...
  var ReactStyleSheets = {
    setOptions: function (options) {
      if (optionsSet) {
        warn('It is not advised that you set ReactStyleSheets options more than once.');
      }

      if (typeof options.vendorPrefixes !== 'undefined') {
        if (typeof options.vendorPrefixes !== 'object' || Array.isArray(options.vendorPrefixes)) {
          throw new Error(
            'vendorPrefixes must be an object with style property names as keys, and arrays of prefixes as values.'
          );
        } else {

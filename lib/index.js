...
(function () {
  ...
  var shouldBeCommaSeparated = /^(fontFamily)$/;
  var shouldNotHaveValueSuffix = /^(opacity|animationIterationCount)$/;
  var shouldHaveTimeSuffix = /^(transition|transitionDuration|transitionDelay|animation|animationDuration|animationDelay)$/;
  var whitespace = /[\n\r\s]/g;
  var indent = '  ';
  ...
  function formatValue (key, value) {
    if (Array.isArray(value)) {
      return value.map(formatValue.bind(null, key)).join(shouldBeCommaSeparated.test(key) ? ', ' : ' ');
    } else if (typeof value === 'number') {
      if (value === 0 || shouldNotHaveValueSuffix.test(key)) {
        return value;
      }

      if (shouldHaveTimeSuffix.test(key)) {
        return value + 'ms';
      }
  ...
  var ReactStyleSheets = {
    ...
    createUniqueClassStyles: function (styles) {
      ...
      for (var className in styles) {
        var obfuscatedClassName = obfuscate ? generateClassName(className) : className;

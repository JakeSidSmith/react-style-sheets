...
(function () {
  ...
  function formatValue (key, value) {
    if (Array.isArray(value)) {
      return value.map(formatValue.bind(null, key)).join(commaSeparated.test(key) ? ', ' : ' ');
  ...
  function createStyles (tag, style) {
    ...
    if (tag.indexOf('-') >= 0) {
      throw new Error(
        messagePrefix + 'Found "-" in a tag or class name. Tag and class names must be camelcase.'
      );
    }

    ...
    for (var key in style) {
      ...
      if (key in vendorPrefixes) {
        ...
        for (var i = 0; i < vendorPrefixes[key].length; i += 1) {
          ...
          prefix += indentation + indent + toSpinalCase(vendorKey) + ': ' +
            formatValue(key, value) + ';\n';
        }

        prefix += indentation + indent + toSpinalCase(key) + ': ' +
          formatValue(key, value) + ';\n';
        ...
        for (var subKey in value) {
          ...
          if (subKey === 'vertical' || subKey === 'horizontal' || subKey === 'all') {
            if (subKey !== 'horizontal') {
              keySubKey = key + '-' + 'top';
              prefix += indentation + indent + toSpinalCase(keySubKey) + ': ' +
